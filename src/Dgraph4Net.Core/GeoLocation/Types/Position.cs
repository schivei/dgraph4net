...
{
    private static readonly DoubleTenDecimalPlaceComparer s_doubleComparer = new();
    ...
    public static bool operator ==(Position left, Position right)
    {
        ...
        if (!s_doubleComparer.Equals(left.Latitude, right.Latitude) ||
            !s_doubleComparer.Equals(left.Longitude, right.Longitude))
        {
            return false;
        }
        return left.Altitude.HasValue == right.Altitude.HasValue &&
               (!left.Altitude.HasValue || s_doubleComparer.Equals(left.Altitude.Value, right.Altitude.Value));
    }
