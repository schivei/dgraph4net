...
public static partial class DgraphExtensions
{
    ...
    public static IEnumerable<Attribute> GetTypeAttributes(this Type type)
    {
        ...
        {
            var baseAttrs = type.BaseType.GetTypeAttributes();
    ...
    {
        var attrs = new HashSet<Attribute>();
        if (prop.PropertyType.HasCustomType())
        {
            prop.PropertyType.GetTypeAttributes().ToList()
                .ForEach(a => attrs.Add(a));
        }
        ...
        {
            foreach (var attr in prop.GetPropertyAttributes())
            {
                var pre = attrs.FirstOrDefault(x => x.GetType() == attr.GetType());
                if (pre is null)
                {
    ...
    public static T GetPropertyAttribute<T>(this PropertyInfo prop) where T : Attribute
    {
        ...
        var attrs = prop.GetPropertyAttributes();
    ...
    {
        var attrs = type.GetTypeAttributes();
    ...
    public static StringBuilder Map(this IDgraph4NetClient dgraph, params Assembly[] assemblies)
    {
        ...
        properties.Where(pp => pp.prop.DeclaringType is not null &&
            ...
            .Select(pp =>
            {
                ...
                var jattr =
                    ...
                    new JsonPropertyNameAttribute(prop.Name.NormalizeName());

                if (jattr.Name == "dgraph.type" || jattr.Name == "uid")
                    return (null, null, null, null, null, null, false);

                var pAttr = prop.GetPropertyAttributes()
                ...
                if (isEnum)
                {
                    var converter = princType.GetTypeAttribute<JsonConverterAttribute>();
                    if (converter != null && typeof(JsonStringEnumConverter).IsAssignableFrom(converter.ConverterType))
                        propType = "string";
                    else
                        propType = "int";
                }
                ...
                var isReference = isReverse || isUid || prop.GetPropertyAttribute<PredicateReferencesToAttribute>() is not null;

                var name = jattr.Name;

                return (name, predicate, type, type.GetTypeAttribute<DgraphTypeAttribute>().Name, propType, prop, isReference);
            }).Where(x => x.name is not null).ToArray();

        var ambiguous = triples.GroupBy(x => x.name)
        ...
        foreach (var predicate in triples.OrderBy(p => p.name).GroupBy(tp => tp.predicate)
        ...
        var ts = triples.OrderBy(p => p.Name).GroupBy(triple => triple.Name)
            .Select(tp =>
            {
                ...
                var typeProperties = tp.Select(pred =>
                {
                    ...
                    var r = prop.GetPropertyAttribute<PredicateReferencesToAttribute>();
                    ...
                    var tn = refType.GetTypeAttribute<DgraphTypeAttribute>().Name;
...
{names}
            ...
            if (predicate.isReversed || predicate.IsReference)
            {
                ...
                if (expand is not null && pt != typeof(Uid))
                {
                    ...
                    if (typeof(IEntity).IsAssignableFrom(pt))
                    {
                        ...
                        {
                            content = ContentIdentation().Replace(content, "  ");
    ...
    [GeneratedRegex("^")]
    private static partial Regex ContentIdentation();
}
