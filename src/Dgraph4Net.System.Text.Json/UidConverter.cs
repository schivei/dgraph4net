...
internal partial class UidConverter : JsonConverter<Uid>
{
    public override Uid Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        {
            JsonValueKind.Object => new((value.HasValue ? value.Value.Deserialize<Dictionary<string, JsonElement>>() : [])["uid"].GetString()),
            JsonValueKind.String => new(value.Value.GetString()),
            JsonValueKind.Number => new(value.Value.GetUInt64()),
            _ => default,
        };
    }
