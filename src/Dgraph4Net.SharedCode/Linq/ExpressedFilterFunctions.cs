...
internal sealed class ExpressedFilterFunctions
{
    ...
    public override string ToString() => $"@filter{_stringBuilder}";
    ...
    private class FilterExpressionVisitor(FilterFunctions functions, ExpressedFilterFunctions expressed) : ExpressionVisitor
    {
        protected override Expression VisitBinary(BinaryExpression node)
        {
            using var _ = defer(() =>
            {
                if (node.NodeType is not (ExpressionType.AndAlso or ExpressionType.OrElse)) return;
                
                expressed.OpenParenthesis();
                Visit(node.Left);
                if (node.NodeType == ExpressionType.AndAlso)
                    ...
                    expressed.Or();
                Visit(node.Right);
                expressed.CloseParenthesis();
            });

            return node;
        }

        private static void ReadConstant(Expression expression, Type targetType, int _, ref object obj)
        {
        ...
        private static void ReadArray(Expression expression, Type _, int __, ref object obj)
        {
        ...
        private static void ReadParameter(Expression expression, Type _, int __, ref object obj)
        {
        ...
        private static void ReadMember(Expression expression, Type _, int __, ref object obj)
        {
        ...
        private static void ReadMethodCall(Expression expression, Type _, int __, ref object obj)
        {
        ...
        protected override Expression VisitUnary(UnaryExpression node)
        {
            using var _ = defer(() =>
            {
                if (node.NodeType is not ExpressionType.Not)
                    return;
