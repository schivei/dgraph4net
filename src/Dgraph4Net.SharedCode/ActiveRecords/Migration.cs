...
public abstract class Migration : AEntity<Migration>, IDgnMigration
{
    ...
    internal async Task<string> GetSchema()
    {
        ...
        var assemblyDir = Path.GetDirectoryName(assemblyPath) ?? throw new("Assembly path not found.");
        ...
        await using var stream = assembly.GetManifestResourceStream(schema) ?? throw new("Schema not found.");
    ...
    protected Migration()
    {
        ...
        if (GeneratedAt == DateTimeOffset.MinValue)
            throw new($"Invalid migration name '{name}'.");
    ...
    dgn.applied_at
        ...
        if (DropPredicates.Any())
        {
            foreach (var pre in DropPredicates)
            {
                ...
                try
                {
                    await txn.MutateWithQuery(new()
                        {
                        DelNquads = new NQuad { Subject = "uid(d)", Predicate = pre, ObjectValue = new() { DefaultVal = "_STAR_ALL" } }.ToByteString(),
                        CommitNow = true
        ...
        else if (!dgnm.Uid.IsConcrete)
        {
            ...
            catch (Exception ex)
            {
                ...
                throw new($"Error on mutate with: {mut}", ex);
            }
        }
    }
}
