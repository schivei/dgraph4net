...
namespace Dgraph4Net.OpenIddict.Stores
{
        ...
        where TToken : OpenIddictDgraphToken, IEntity, new()
    {
        ...
        public virtual ValueTask<ImmutableDictionary<string, JsonElement>> GetPropertiesAsync(TToken token, CancellationToken cancellationToken)
        {
            ...
            var document = JsonConvert.DeserializeObject<Dictionary<string, JsonElement>>(ByteString.CopyFrom(token.Properties).ToStringUtf8());
            ...
            ImmutableDictionary<string, JsonElement> properties, CancellationToken cancellationToken)
        {
            ...
            token.Properties = ByteString.CopyFromUtf8(JsonConvert
        ...
        public virtual async ValueTask UpdateAsync(TToken token, CancellationToken cancellationToken)
        {
            ...
            var mu = new Mutation
            {
                CommitNow = true,
                Cond = "@if(eq(len(u, 1)))",
                SetJson = ByteString.CopyFromUtf8(JsonConvert.SerializeObject(token, new JsonSerializerSettings
                {
