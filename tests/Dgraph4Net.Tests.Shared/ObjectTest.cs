...
public class ObjectTest : ExamplesTest
{
    ...
    public async Task SetObjectTest()
    {
        ...
        try
        {
            ...
            var dob = new DateTimeOffset(new(1980, 01, 01, 23, 0, 0, 0, DateTimeKind.Utc));
            ...
            var alice = new Person
            {
                ...
                Married = true,
                DgraphType = ["Person"],
                Location = new() { Coordinates = [1.1, 2d] },
                Dob = dob,
                Raw = "raw_bytes"u8.ToArray(),
                Friends = [bob, charlie],
                Schools = [
                    new()
                    {
            ...
            var mu = new Mutation
            {
                CommitNow = true, SetJson = aj
            };

            _ = await dg.NewTransaction().Mutate(mu);

            // Assigned uids for nodes which were created would be returned in the response.Uids map.
            var vars = new VarTriples { new("id1", "Alice") };
            var variables = vars.ToDictionary();
            var q = $$"""
                      query Me({{vars.ToQueryString()}}){
    ...
    public async Task TxnQueryVariablesTest()
    {
        ...
        try
        {
            ...
            var p = new Person { DgraphType = ["Person"] };
    ...
    public async Task TxnMutateTest()
    {
        ...
        try
        {
            ...
            var bob = new Person { Age = 24, DgraphType = ["Person"] };

            bob.Name += "Bob";

            var charlie = new Person { Age = 29, DgraphType = ["Person"] };
            ...
            var p = new Person
            {
                ...
                Schools = [new() { Name = "Crown Public School", DgraphType = ["Institution"] }],
            };
    ...
    public async Task TxnQueryUnmarshalTest()
    {
        ...
        try
        {
            ...
            _ = await txn.Mutate(mu);

            // Assigned uids for nodes which were created would be returned in the response.Uids map.
            const string puid = "Alice";
            var variables = new Dictionary<string, string> { { "$id", puid } };
            const string q = """
    ...
    public async Task TxnQueryBestEffortTest()
    {
        try
        {
            ...
            var response = await dg.NewTransaction().Mutate(new()
            {
    ...
    public async Task TxnMutateVarsTest()
    {
        ...
        try
        {
            ...
            var p = new Person
            {
                ...
                Married = true,
                DgraphType = ["Person"],
                Location = new() { Coordinates = [1.1d, 2] },
                Raw = "raw_bytes"u8.ToArray(),
                Friends =
                [
                    new() {Age = 24, DgraphType = ["Person"] },
                    new() {Age = 29, DgraphType = ["Person"] }
                ],
                Schools = [new() { Name = "Crown Public School", DgraphType = ["Institution"] }],
            };
            ...
            mu = new() { CommitNow = true, DelNquads = ByteString.CopyFromUtf8(nq) };
